...
def get_dataflow(egocap_data_paths):
    """
    This function initializes the tensorpack dataflow and serves generator
    for training operation.

    :param egocap_data_paths: paths to the egocap files: annotation file and folder with images
    :return: dataflow object
    """
    df = EgoCapDataFlow((368, 368), egocap_data_paths)
    df.prepare()
...
if __name__ == '__main__':
    ...
    df = EgocapDataFlow((368, 368), EgoCapDataPaths(annot_path, img_dir))#, select_ids=[1000])
    df.prepare()
    df = MapData(df, read_img)
    df = MapData(df, gen_mask)
    df = MapData(df, augment)
