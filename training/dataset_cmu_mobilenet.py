...
ALL_PAF_MASK = np.repeat(
    np.ones((7, 7, 1), dtype=np.uint8), 38, axis=2)

ALL_HEATMAP_MASK = np.repeat(
    np.ones((7, 7, 1), dtype=np.uint8), 19, axis=2)
...
AUGMENTORS_LIST = [
        ...
        CropAug(224, 224, center_perterb_max=40, border_value=(128, 128, 128),
                 mask_border_val=1),
...
def build_sample(components):
    ...
    if meta.mask is None:
        ...
        mask_paf = create_all_mask(meta.mask, 38, stride=32)
        mask_heatmap = create_all_mask(meta.mask, 19, stride=32)

    heatmap = create_heatmap(JointsLoader.num_joints_and_bkg, 7, 7,
                             meta.aug_joints, 7.0, stride=32)

    pafmap = create_paf(JointsLoader.num_connections, 7, 7,
                        meta.aug_joints, 1, stride=32)
...
def get_dataflow(coco_data_paths):
    ...
    """
    df = CocoDataFlow((224, 224), coco_data_paths)
    df.prepare()
...
if __name__ == '__main__':
    ...
    df = CocoDataFlow((224, 224), COCODataPaths(annot_path, img_dir))#, select_ids=[1000])
    df.prepare()
    df = MapData(df, read_img)
    df = MapData(df, gen_mask)
    df = MapData(df, augment)
