...
class GroundTruth:
    def __init__(self, joints):
        self.joints = joints
        self.bones_size = {
            bone_name: self.joints[bone_edges[0]].distance(
                self.joints[bone_edges[1]])
            for (bone_name, bone_edges) in bones_list.items() }

    def __str__(self):
        s = ''
        for bone_name, bone_value in self.bones_size.items():
            s += str(bone_name) + ': ' + str(bone_value) + '\n'
        return s
...
class Render:
    def __init__(self, width, height, camera, screen_world_joints, joints):
        pygame.init()
    ...
    def run(self):
        while True:
            ...
            if self.camera is not None and self.render_switches['camera']:
                self.draw_camera()

            if self.screen_world_joints is not None:
                self.draw_rays(self.screen_world_joints.joints)
                self.draw_skeleton(
                    self.screen_world_joints.joints, type='screen_world_joints')
...
screen_world_joints = GroundTruth(camera.get_screen_world_joints(
        screen_joints=screen_joints))
...
Render(1200, 900, camera, screen_world_joints, ground_truth.joints).run()
